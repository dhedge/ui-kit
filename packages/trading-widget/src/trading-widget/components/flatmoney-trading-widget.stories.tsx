...
const FLAT_MONEY_PRESET = {
  ...
  components: {
    ActionButton: ({ onClick, disabled, loading, children }) => {
      return (
        <button
          onClick={onClick}
          disabled={disabled || loading}
          className={classNames(
            'dtw-border-0 dtw-w-full dtw-font-medium dtw-overflow-hidden dtw-bg-clip-text dtw-py-4 dtw-px-6 dtw-text-[color:#161213] dtw-rounded-xl',
            {
              'dtw-opacity-50 dtw-flex dtw-items-center dtw-justify-center dtw-gap-1':
                loading,
            },
          )}
          style={{
